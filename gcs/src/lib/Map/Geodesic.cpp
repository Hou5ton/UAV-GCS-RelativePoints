...
using namespace std;
  ...
  const double MGeodesic::eps2 = sqrt(numeric_limits<double>::min());
  const double MGeodesic::tol0 = numeric_limits<double>::epsilon();
  const double MGeodesic::tol1 = 100 * tol0;
  const double MGeodesic::tol2 = sqrt(numeric_limits<double>::epsilon());
  const double MGeodesic::xthresh = 1000 * tol2;

  MGeodesic::MGeodesic(double a, double r)
    : _a(a)
    , _r(r)
    , _f(_r != 0 ? 1 / _r : 0)
    , _f1(1 - _f)
  ...
  double MGeodesic::SinSeries(double sinx, double cosx,
  ...
  MGeodesicLine MGeodesic::Line(double lat1, double lon1, double azi1) const throw() {
    return MGeodesicLine(*this, lat1, lon1, azi1);
  }

  double MGeodesic::Direct(double lat1, double lon1, double azi1, double s12,
                              double& lat2, double& lon2, double& azi2) const throw() {
    MGeodesicLine l(*this, lat1, lon1, azi1);
    return l.Position(s12, lat2, lon2, azi2);
  }

  double MGeodesic::Inverse(double lat1, double lon1, double lat2, double lon2,
    ...
    sbet2 = _f1 * sin(phi);
    cbet2 = fabs(lat2) == 90 ? eps2 : cos(phi);
    ...
    if (meridian) {
      ...
      // sig12 = sig2 - sig1
      sig12 = atan2(max(csig1 * ssig2 - ssig1 * csig2, double(0)),
    ...
    if (!meridian &&
      ...
      // Newton's method
      double ssig1 = 0.0, csig1 = 0.0, ssig2 = 0.0, csig2 = 0.0, k1 = 0.0;
      double ov = 0;
      unsigned numit = 0;
      for (unsigned trip = 0; numit < maxit; ++numit) {
        ...
        if (fabs(v) <= eps2 || !(trip < 1)) {
          if (fabs(v) > max(tol1, ov))
            numit = maxit;
          break;
        }
        double
    ...
    azi1 = 0 - atan2(- swapp * lonsign * salp1,
                     + swapp * latsign * calp1) * 180 / pi;
    azi2 = 0 - atan2(- swapp * lonsign * salp2,
                     + swapp * latsign * calp2) * 180 / pi;
	
	if (azi1<0) azi1 = 360.0 + azi1;
	if (azi2<0) azi2 = 360.0 + azi2;
  ...
  double MGeodesic::CalcSpheroidalArea(MPtArray *pta) const throw()
  {
    if (!pta) return 0;
    if (pta->Size == 0) return 0;
	const double r_auth = _b * sqrt(1 + (2.0/3 + (3.0/5 + 4 * _e2 / 7) * _e2) * _e2),
    to_auth_2 = (-1.0/3 + (-31.0/180 - 59 * _e2 / 560) * _e2) * _e2,
    to_auth_4 = (17.0/360 + 61 * _e2 / 1260) * pow(_e2, 2),
    to_auth_6 = -383 * pow(_e2, 3) / 45360;
	double tau = 0, tau_i, yRad, aziPlus, aziMinus, nextAziMinus, lastAziPlus, nothing;
	int ptCount;
	MPtArray equivPoints;
	equivPoints.AddTail(pta->GetAt(0));
	yRad = equivPoints.GetAt(0)->y * pi / 180;
	yRad += to_auth_2 * sin(2 * yRad) + to_auth_4 * sin(4 * yRad) + to_auth_6 * sin(6 * yRad);
	equivPoints.GetAt(0)->y = yRad * 180 / pi;
	for(int i = 1; i < pta->Size; ++i)
	{
		if(DLINASG(pta->GetAt(i - 1), pta->GetAt(i)) > 0.0001)
		{
			equivPoints.AddTail(pta->GetAt(i));
			yRad = pta->GetAt(i)->y * pi / 180;
			yRad += to_auth_2 * sin(2 * yRad) + to_auth_4 * sin(4 * yRad) + to_auth_6 * sin(6 * yRad);
			equivPoints.GetAt(equivPoints.Size - 1)->y = yRad * 180 / pi;
		}			
	}
	if(DLINASG(equivPoints.GetAt(0), equivPoints.GetAt(equivPoints.Size - 1)) > 0.0001)
		equivPoints.AddTail(equivPoints.GetAt(0));
	ptCount = equivPoints.Size;
	Inverse(equivPoints.GetAt(0)->y, equivPoints.GetAt(0)->x, equivPoints.GetAt(ptCount - 2)->y, equivPoints.GetAt(ptCount - 2)->x, nothing, aziMinus, lastAziPlus);
	aziMinus *= pi / 180;
	lastAziPlus *= pi / 180;
	if(lastAziPlus < pi)
		lastAziPlus += pi;
		...
		lastAziPlus -= pi;
	Inverse(equivPoints.GetAt(0)->y, equivPoints.GetAt(0)->x, equivPoints.GetAt(1)->y, equivPoints.GetAt(1)->x, nothing, aziPlus, nextAziMinus);
	aziPlus *= pi / 180;
	nextAziMinus *= pi / 180;
	if(nextAziMinus < pi)
		nextAziMinus += pi;
		...
		nextAziMinus -= pi;
	tau_i = 0.5 - (aziMinus - aziPlus) / (2 * pi);
	tau_i -= floor(tau_i + 0.5);
	tau += tau_i;
	for(int i = 1; i < ptCount - 2; ++i)
	{
		aziMinus = nextAziMinus;
		Inverse(equivPoints.GetAt(i)->y, equivPoints.GetAt(i)->x, equivPoints.GetAt(i + 1)->y, equivPoints.GetAt(i + 1)->x, nothing, aziPlus, nextAziMinus);
		aziPlus *= pi / 180;
		nextAziMinus *= pi / 180;
		if(nextAziMinus < pi)
			nextAziMinus += pi;
	...
	tau_i = 0.5 - (nextAziMinus - lastAziPlus) / (2 * pi);
	tau_i -= floor(tau_i + 0.5);
	tau += tau_i;
    return 2 * pi * (1 - fabs(tau)) * pow(r_auth, 2);
  }
  void MGeodesic::Lengths(double k1, double sig12,
  ...
  void MGeodesic::Evolute(double R, double z, double& s, double& c) throw() {
    ...
    if ( !(q == 0 && r <= 0) ) {
      ...
      if (disc >= 0) {
        ...
        // There are three possible double solutions for u depending on the
        ...
        u += 2 * fabs(r) * cos((2 * pi + ang) / double(3));
      }
      double
  ...
  void MGeodesic::InverseStart(double sbet1, double cbet1, double sbet2, double cbet2,
    ...
    calp1 = clam12 >= 0 ?
      sbet12 * (slam12 < double(0.1) ? sqrt(1 - _e2 * sq(cbet1)) : double(1))
      + cbet2 * sbet1 * sq(slam12) / (1 + clam12) :
      sbet12a - cbet2 * sbet1 * sq(slam12) / (1 - clam12);

    double
      ssig12 = sqrt(pow(salp1,2)+pow(calp1,2)),
      csig12 = sbet1 * sbet2 + cbet1 * cbet2 * clam12;

    if (csig12 >= 0 || ssig12 >= 3 * fabs(_f) * pi * sq(cbet1)) {
      ...
      if (_f >= 0) {            // In fact f == 0 does not get here
        // x = dlong, y = dlat
        {
          ...
          lamscale = -cbet1 * etaFactor(_f, k1) * pi;
        }
        ...
        // Inverse.
        Lengths(_n, pi + bet12a, sbet1, -cbet1, sbet2, cbet2,
                cbet1, cbet2, dummy, x, m0, tc, zc);
        x = -1 + x/(_f1 * cbet1 * cbet2 * m0 * pi);
      ...
      if (y > -100 * tol1 && x >  -1 - xthresh) {
        // strip near cut
        if (_f >= 0) {
          salp1 = min(double( 1), -x); calp1 = - sqrt(1 - sq(salp1));
        } else {
          calp1 = max(double(-1),  x); salp1 =   sqrt(1 - sq(calp1));
        }
      } else {
        ...
        double
          omg12a = lamscale * ( _f >= 0
                                ? sqrt(pow(y,2)+pow( salp2 + x,2)) * salp2
                                : sqrt(pow(x,2)+ pow(-calp2 + y,2)) * calp2 ),
  ...
  double MGeodesic::Lambda12(double sbet1, double cbet1, double sbet2, double cbet2,
                                double salp1, double calp1,
                                ...
                                double tc[], double zc[], double ec[]) const throw() {
    ...
    double
      ...
      salp0 = salp1 * cbet1,
      calp0 = sqrt(pow(calp1,2)+ pow(salp1 * sbet1,2)); // calp0 > 0
    ...
    omg12 = atan2(max(comg1 * somg2 - somg1 * comg2, double(0)),
  ...
  MGeodesicLine::MGeodesicLine(const MGeodesic& g,
    ...
    , _f1(g._f1)
  {
    azi1 = MGeodesic::AngNormalize(azi1);
    // Guard against underflow in salp0
    azi1 = MGeodesic::AngRound(azi1);
    lon1 = MGeodesic::AngNormalize(lon1);
    _lat1 = lat1;
    _lon1 = lon1;
    _azi1 = azi1;
    // alp1 is in [0, pi]
    ...
    // Ensure cbet1 = +eps at poles
    sbet1 = _f1 * sin(phi);
    cbet1 = fabs(lat1) == 90 ? MGeodesic::eps2 : cos(phi);
    MGeodesic::SinCosNorm(sbet1, cbet1);

    // Evaluate alp0 from sin(alp1) * cos(bet1) = sin(alp0),
    _salp0 = salp1 * cbet1; // alp0 in [0, pi/2 - |bet1|]
    // Alt: calp0 = hypot(sbet1, calp1 * cbet1).  The following
    // is slightly better (consider the case salp1 = 0).
    _calp0 = sqrt(pow(calp1,2)+ pow(salp1 * sbet1,2));
    ...
    MGeodesic::SinCosNorm(_ssig1, _csig1); // sig1 in (-pi, pi]
    MGeodesic::SinCosNorm(_somg1, _comg1);

    double mu = sq(_calp0);
    _u2 = mu * g._ep2;
    double k1 = _u2 / (2 * (1 + sqrt(1 + _u2)) + _u2);
    _taufm1 =  MGeodesic::tauFactorm1(k1);
    _zetfm1 =  MGeodesic::zetFactorm1(k1);

    MGeodesic::tauCoeff(k1, _tauCoeff);
    MGeodesic::sigCoeff(k1, _sigCoeff);
    MGeodesic::zetCoeff(k1, _zetCoeff);
    MGeodesic::etaCoeff(g._f, k1, _etaCoeff);

    _dtau1 = MGeodesic::SinSeries(_ssig1, _csig1, _tauCoeff, ntau);
    _dzet1 = MGeodesic::SinSeries(_ssig1, _csig1, _zetCoeff, nzet);
    {
    ...
    _etaf = _salp0 * MGeodesic::etaFactor(g._f, k1);
    _dlam1 = MGeodesic::SinSeries(_ssig1, _csig1, _etaCoeff, neta);
  }

  double MGeodesicLine::Position(double s12, double& lat2, double& lon2, double& azi2)
  {
    if (!Init())
      // Uninitialized
      return 0;
    ...
    double
        ...
        c = cos(tau12);
      // tau2 = tau1 + tau12
      dtau2 = - MGeodesic::SinSeries(_stau1 * c + _ctau1 * s,
                                    _ctau1 * c - _stau1 * s,
                                    _sigCoeff, nsig);
    ...
    cbet2 = sqrt(pow(_salp0,2)+pow(_calp0 * csig2,2));
    // tan(omg2) = sin(alp0) * tan(sig2)
    ...
    lam12 = omg12 + _etaf *
      ( sig12 +
        (MGeodesic::SinSeries(ssig2, csig2, _etaCoeff, neta)  - _dlam1));
    lon12 = lam12 / (pi / 180);
    // Can't use AngNormalize because longitude might have wrapped multiple
    // times.
    lon12 = lon12 - 360 * floor(lon12/360 + double(0.5));
    lat2 = atan2(sbet2, _f1 * cbet2) / (pi / 180);
    lon2 = MGeodesic::AngNormalize(_lon1 + lon12);
    // minus signs give range [-180, 180). 0- converts -0 to +0.
    azi2 = 0 - atan2(-salp2, calp2) / (pi / 180);

//    double
//      et = (1 + _taufm1) * (dtau2 - _dtau1),
//      ez = (1 + _zetfm1) * (MGeodesic::SinSeries(ssig2, csig2, _zetCoeff, nzet)
//                            - _dzet1);
  ...
  double MGeodesic::tauFactorm1(double k1) throw() {
    double
      k2 = sq(k1),
      t;
    switch (tauord/2) {
  ...
  void MGeodesic::tauCoeff(double k1, double t[]) throw() {
    double
      k2 = sq(k1),
      d = k1;
    switch (ntau) {
...
//      STATIC_ASSERT(ntau >= 0 && ntau <= 8, "Bad value of ntau");
  ...
  void MGeodesic::sigCoeff(double k1, double tp[]) throw() {
    double
      k2 = sq(k1),
      d = k1;
    switch (nsig) {
...
//      STATIC_ASSERT(nsig >= 0 && nsig <= 8, "Bad value of nsig");
  ...
  double MGeodesic::zetFactorm1(double k1) throw() {
    double
      k2 = sq(k1),
      t;
    switch (zetord/2) {
...
//      STATIC_ASSERT(zetord >= 0 && zetord <= 8, "Bad value of zetord");
  ...
  void MGeodesic::zetCoeff(double k1, double z[]) throw() {
    double
      k2 = sq(k1),
      d = k1;
    switch (nzet) {
...
//      STATIC_ASSERT(nzet >= 0 && nzet <= 8, "Bad value of nzet");
  ...
  double MGeodesic::etaFactor(double f, double k1) throw() {
    double
      fp = (f - k1) / (1 - k1),
...
//      STATIC_ASSERT(etaord >= 0 && etaord <= 8, "Bad value of etaord");
  ...
  void MGeodesic::etaCoeff(double f, double k1, double h[]) throw() {
    double
      fp = (f - k1) / (1 - k1),
      nu = fp !=0 ? 2 * k1 / fp : 2, // Correct limit is mu / (1 - mu/2)
      nu2 = sq(nu);
