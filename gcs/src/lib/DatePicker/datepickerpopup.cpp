...
class DatePickerPopupPrivate {
   ...
   // QLabel *label_2;
    ...
    {
        Q_Q(DatePickerPopup);
        //label_1 = new QLabel(q);
        //label_2 = new QLabel(q);

        calendar_widget_1 = new DatePickerCalendar(q,QObject::tr("From"));
        calendar_widget_2 = new DatePickerCalendar(q,QObject::tr("To"));
        ...
        inputs_layout->setContentsMargins(QMargins());
        ...
        //inputs_layout->addWidget(label_1, iCount, 0);
        //inputs_layout->addWidget(label_2, iCount, 1);
        //iCount++;

        inputs_layout->addWidget(calendar_widget_1, iCount, 0);
        inputs_layout->addWidget(calendar_widget_2, iCount, 1);
        iCount++;

        inputs_layout->addWidget(time_edit_1, iCount, 0);
        inputs_layout->addWidget(time_edit_2, iCount, 1);

        /*QHBoxLayout *time_layout = new QHBoxLayout(q);
        time_layout->setSizeConstraint(QLayout::SetFixedSize);
        time_layout->addWidget(label_from, 0);
        time_layout->addWidget(label_to, 0);*/
    ...
    d_ptr(new DatePickerPopupPrivate(this))
{
    ...
    connect(d->footer, &DatePickerPopupFooter::applyClicked,this,&DatePickerPopup::onApplyClicked);
    connect(d->footer, &DatePickerPopupFooter::cancelClicked,this,&DatePickerPopup::onCancelClicked);
...
void DatePickerPopup::onApplyClicked()
{
    emit sigApplyClicked();
}

void DatePickerPopup::onCancelClicked()
{
...
void DatePickerPopup::setDatePeriodFull(const QDate &begin, const QDate &end)
{
    Q_D(DatePickerPopup);
    //d->label_1->setText(QObject::tr("From") + " " + begin.toString());
    //d->label_2->setText(QObject::tr("To")+ " " + end.toString());
    d->calendar_widget_1->setDatePeriodFull(begin, end);
    d->calendar_widget_2->setDatePeriodFull(begin, end);
}
