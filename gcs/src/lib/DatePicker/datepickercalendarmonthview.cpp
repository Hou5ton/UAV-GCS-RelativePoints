...
class DatePickerCalendarMonthViewPrivate {
    ...
    QDate period_begin;
    QDate period_end;
    QDate period_begin_full;
    QDate period_end_full;
        ...
        q_ptr(q)
    {
        ...
        normal_background = QColor(Qt::black);
        normal_foreground = QColor(Qt::white);

        invalid_background = QColor(Qt::black);
        invalid_foreground = QColor(Qt::darkGray);
    ...
    d_ptr(new DatePickerCalendarMonthViewPrivate(this))
{
    ...
    cell_text_format.setBackground(QBrush(Qt::black));
    cell_text_format.setForeground(/*palette().windowText()*/QBrush(Qt::white));
...
void DatePickerCalendarMonthView::setDatePeriodFull(const QDate &begin, const QDate &end)
{
    Q_D(DatePickerCalendarMonthView);
    d->period_begin_full = begin;
    d->period_end_full = end;
}
...
void DatePickerCalendarMonthView::paintCell(QPainter *painter, const QRect &rect, const QDate &date) const
{
    ...
    bool bRet1 = date < minimumDate();
    bool bRet2 = date > maximumDate();
    bool bRet3 = date.month() != monthShown();
    bool bRet4 = date.month() != monthShown();

    bool is_invalid_date = (bRet1 || bRet2 || bRet3 ||bRet4);
    ...
    if (d->period_begin_full.isValid() && d->period_end_full.isValid()){
        is_highlighted_date = ((date >= d->period_begin_full) && (date <= d->period_end_full));
    }

    QColor background_color;
    if(is_invalid_date){
        background_color = d->invalid_background;
    }else if(is_highlighted_date){
        background_color = d->highlighted_background;
    }else{
       background_color = d->normal_background;
    }

    QColor foreground_color;
    if(is_invalid_date){
        foreground_color = d->invalid_foreground;
    }else if(is_highlighted_date){
        foreground_color = d->highlighted_foreground;
    }else{
       foreground_color = d->normal_foreground;
    }
