...
void VideoThread::setQualityLevel(int qualityLevel)
{
    m_qualityLevel = 25 - qualityLevel;
}

int VideoThread::getQualityLevel() const
{
    return m_qualityLevel;
}

void VideoThread::setCompressionLevel(int compressionLevel)
{
    m_compressionLevel = compressionLevel;
}

int VideoThread::getCompressionLevel() const
{
    return m_compressionLevel;
}

void VideoThread::setMaxSizeTime(unsigned long long maxSizeTime)
{
    m_maxSizeTime = maxSizeTime;
}

unsigned long long VideoThread::getMaxSizeTime() const
{
    return m_maxSizeTime;
}
...
static void on_pad_added_urisourcebin(GstElement *el1, GstPad *pad, GstElement *el2)
{
    ...
    //without this, if we link audio stream first, pipeline will be stuck without audio sink
    if (mediaType != "audio")
...
static gboolean my_bus_callback(GstBus *bus, GstMessage *message, gpointer data)
{
    ...
    case GST_MESSAGE_ERROR: {
        //check rec error
        StreamContext *context = (StreamContext *) data;
        //common problems occurred from muxer and watchdog
        if (message->src == GST_OBJECT(context->recWatchdog)
            || message->src == GST_OBJECT(context->recMuxer)) {
...
void VideoThread::run()
{
    ...
    auto bus = gst_pipeline_get_bus(GST_PIPELINE(context->pipeline.get()));
    auto gc = g_main_context_default();
    GSource *source = gst_bus_create_watch(bus);
    g_source_attach(source, gc);
    g_source_set_callback(source, (GSourceFunc) my_bus_callback, context.get(), nullptr);
...
GstElement *VideoThread::createSourceElement(StreamContext *context)
{
    ...
    if (m_uri.contains("avf://")) {
        ...
        QString address = u.host();
        int port = u.port();
        QString query = u.query();
        g_object_set(result, "address", address.toStdString().c_str(), nullptr);
        g_object_set(result, "port", port, nullptr);

        std::shared_ptr<GstCaps> caps;
        if (query.contains("codec=h265"))
            caps = getCapsForUdpSrc("H265");
        else if (query.contains("codec=h264"))
            caps = getCapsForUdpSrc("H264");
        else if (query.contains("codec=raw-jpeg2000"))
            caps = getCapsForUdpSrc("raw-jpeg2000");
...
{
    QString filename = GstPlayer::getMediaFileName(GstPlayer::mtVideo,GstPlayer::getCurrentDateTime());
    ...
    context->recWatchdog = gst_element_factory_make("watchdog", nullptr);
    context->recSink = gst_element_factory_make("splitmuxsink", nullptr);

    if (!context->recQueue || !context->recMuxer || !context->recWatchdog || !context->recSink
    ...
    g_object_set(G_OBJECT(context->recMuxer), "fragment-duration", 1000, "fragment-mode", 1, nullptr);
    g_object_set(G_OBJECT(context->recWatchdog), "timeout", 10000, nullptr);

    unsigned long long maxSizeTime = m_maxSizeTime;
    g_object_set(G_OBJECT(context->recSink),
                 "location",
    ...
    bool syncResult = gst_element_sync_state_with_parent(context->recQueue)
                      && gst_element_sync_state_with_parent(context->recWatchdog)
...
void VideoThread::closeWriter(StreamContext *context)
{
    ...
    gst_element_set_state(context->recWatchdog, GST_STATE_NULL);
...
{
    std::string s;
    if (codec == "raw-jpeg2000") {
        s = "image/x-jpc";
    } else {
        s = "application/x-rtp,media=video,clock-rate=90000,encoding-name=" + codec;
    }
