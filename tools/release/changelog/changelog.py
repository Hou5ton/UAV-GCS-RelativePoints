    ...
    def __init__(self, commits):
        ...
        for commit in commits:
            ...
            if len(commit.comment) > 0:
                self.comments.append('**'+commit.subject.split('(')[0].strip()+'**\n\n' + commit.comment)
...
class Changelog:
    def __init__(self, remote, token):
        ...
        print('Deploying project \'' + self.repo.working_tree_dir.split('/')[-1] + '\'...')
        ...
        self.date = datetime.datetime.fromtimestamp(self.repo.head.commit.committed_date)
        ...
        deploy_repo_dir = os.path.join(self.repo.working_tree_dir, '..', remote_name)
        if not os.path.exists(deploy_repo_dir):
            print('Clone \'{}\'...'.format(remote_name))
            self.deploy_repo = git.Repo.clone_from('https://{0}@github.com/{1}.git'.format(token, self.remote), deploy_repo_dir)
        else:
        ...
        version_pub = self.deploy_repo.git.describe('--abbrev=0', '--tags', '--always', '--match=*.*')
    ...
    def update(self):
        ...
        notes_file = os.path.join(notes_path, 'release-{}.md'.format(self.version))
    ...
    def update_changes(self, prev_ref):
        ...
        if len(changes) == 0:
            changes = 'Security updates and latest firmware `{}`'.format(self.date.strftime('%x'))

        changelog_file = os.path.join(self.deploy_repo.working_tree_dir, 'CHANGELOG.md')
        ...
        if prev_ref != '':
            changelog_entry_header += \
                '  \nDiff: [uavos/apx](https://github.com/uavos/apx/compare/' + prev_ref + '...' + self.repo.head.commit.hexsha + ')'
    ...
    def publish(self):
        ...
        if len(self.deploy_repo.index.diff('HEAD')) > 0:
            ...
            print('Deploy repo already committed (' + self.deploy_repo.head.commit.message.strip() + ')')
            assert self.deploy_repo.head.commit.message.strip() == deploy_msg

        # create tags
        self.deploy_repo.create_tag('-s', self.version, force=True, message=deploy_msg)
